...
class CO2FootprintObserver implements TraceObserver {
    ...
    private synchronized void aggregateRecords(TraceRecord traceRecord) {
        ...
        final CO2Record co2Record = co2FootprintComputer.computeTaskCO2footprint(traceRecord)
    ...
    void onFlowComplete() {
        ...
        if (workflowStats) {
            ...
            reportFile.addEntries(workflowStats, co2FootprintComputer, config, version, session)
            reportFile.write()
        ...
        log.info(
            ...
            "causing ${workflowStats.value.store['co2e']} of CO2 equivalents to be released into the athmosphere."
        )
    }
