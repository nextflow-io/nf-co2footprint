    ...
    def computeCI() {
        ...
        if (this.location) {
            ...
            if (this.emApiKey && this.emApiKey instanceof String) {
                ...
                log.info(Markers.silentUnique, "Electricity Maps API key is set. Attempting to retrieve real-time carbon intensity.")
            ...
            log.warn(Markers.silentUnique, "No location provided. Attempting to retrieve GLOBAL carbon intensity value.")
        }
        // Fallback to the global default value if no value is found for the location
        if (ci == null) {
            ci = this.ciData.findCiInMatrix('GLOBAL')
