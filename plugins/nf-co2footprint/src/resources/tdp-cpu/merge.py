...
def read_and_process_amd(file_path):
    processors = dict()
...
def read_and_process_ampere_altra(file_path):
    processors = dict()
...
def read_and_process_ampere_one(file_path):
    processors = dict()
...
def read_and_process_intel(file_path):
    processors = dict()
...
processors["AMD"] = read_and_process_amd("AMD/amd-all-specification.csv")
processors["Altere"] = read_and_process_ampere_altra("AMPERE/ampere-altra-specification.csv")
processors["Altere"].update(read_and_process_ampere_one("AMPERE/ampere-one-specification.csv"))

processors["Intel"] = dict()
for file in os.listdir('Intel'):
    file_path = os.path.join('Intel', file)
    processors["Intel"].update(read_and_process_intel(file_path))


## Overwrite / Append with GreenAlgorithms
...
try:
    with open("GreenAlgorithms/TDP_cpu.v2.2.updated.csv", 'r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        ...
        for row in csv_reader:
            try:
                ...
                processors[manufacturer][processor_model] = processor
            except ValueError as ve:
                print(f"Error processing row {row}: {ve}")
except FileNotFoundError:
    print(f"File not found: {file_path}")
