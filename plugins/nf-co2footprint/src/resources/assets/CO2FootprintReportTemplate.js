...
$(function () {
  ...
  var completed_date = moment(
    $("#workflow_complete").text(),
    "ddd MMM DD HH:mm:ss .* YYYY",
  );
  if (completed_date.isValid()) {
    $("#completed_fromnow").html(
      "completed " + completed_date.fromNow() + ", ",
    );
  }
  ...
  for (let processName in window.data.summary) {
    ...
    for (let metricName in metrics) {
      ...
      if (metrics[metricName]["min"] == metrics[metricName]["max"]) {
        ...
        window.statsByProcess[processName][metricName] = [
          metrics[metricName]["min"],
        ];
      } else {
        ...
        window.statsByProcess[processName][metricName] = [
          ...
          "q3",
          "max",
        ].map((stat) => metrics[metricName][stat]);
      }
      if (metricName == "time") {
        window.statsByProcess[processName][metricName] = window.statsByProcess[
          processName
        ][metricName].map(function (d, i) {
  ...
  function plot_resource_usage() {
    ...
    for (var processName in window.statsByProcess) {
      ...
      for (const [i, suffix] of ["", "_non_cached"].entries()) {
        ...
        plot_data[`total${suffix}`].push({
          x: processName,
          y: stats[`energy${suffix}`]?.map((v) => v * 1000) ?? null,
          name: processName,
          type: "box",
          boxmean: true,
          boxpoints: false,
    ...
    var layout = {
      title: {
        text: "CO<sub>2</sub> emission & energy consumption",
      },
      legend: {
        x: 1.1,
      },
